...
- `GET /api/session` liefert der angemeldeten Extension `{ user, access: { ai, tableauApi } }`. Admins verwalten Identitäten über `GET /api/admin/user-access` und `PUT /api/admin/user-access/:id` mit `{ ai, tableauApi, admin? }` — `admin` wird nur vom initialen Admin übernommen (delegierte Admins: `403 initial_admin_required`, ein unverändert mitgesendeter Wert ist erlaubt).
