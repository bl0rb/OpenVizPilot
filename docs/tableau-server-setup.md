...
- Secret-Werte stehen weder in Settings, API-Antworten noch Logs. Im Web gespeicherte Secrets liegen AES-256-GCM-verschlüsselt in der Datenbank (Schlüssel: SHA-256 von `OVP_SECRET_KEY`); ohne diese Umgebungsvariable sind die Secret-Felder deaktiviert und nur Env-Secret-Referenzen nutzbar. Mit `OVP_SECRET_KEY` liegt auch der private EAS-Schlüssel (OAuth 2.0 Trust) verschlüsselt in der Datenbank; ein vorhandener Klartext-Schlüssel wird beim nächsten Lesen umgestellt. Wird `OVP_SECRET_KEY` danach geändert oder entfernt, ist der EAS-Schlüssel nicht mehr lesbar. Logs enthalten nur Operationsstatus, pseudonyme Nutzerkennung, Dauer und Ergebniscode.
